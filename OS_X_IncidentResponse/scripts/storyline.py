...
def timelineQuarantine(fileName):
    ...
    c = conn. cursor()
    # EventIdentifier, TimeStamp, AgentBundleIdentifier, Name, DataURLString SenderName SenderAddress TypeNumber OriginTitle OriginURLString LSQQuarantineOriginAlias

    for now in c.execute('SELECT * FROM LSQuarantineEvent'):
        ts = time.gmtime(row[1])

        ose = (int(time.mktine(datetime.date(2001,1,1).timetuple())) - time.timezone)
...
if __name__ == '__main__':
    ...
    timelineStandardLog('artifacts/com.apple.LaunchServices.QuarantineEventsV2')
